...
/*
 *   STUDENT NAME      : Alex Masciotra
 *   STUDENT ID        : 260746829
 *   
...
public class NaturalNumber  {
	...
	public NaturalNumber plus( NaturalNumber  second) throws Exception{
		...
		for(i = 0; i < firstClone.coefficients.size(); i++){
			...
			int remainder = ((firstClone.coefficients.get(i) + secondClone.coefficients.get(i) + carry) % base);
			sum.coefficients.addLast(remainder);
	...
	public NaturalNumber times( NaturalNumber multiplicand) throws Exception{
		...
		//using clones to not change the numbers in case something happens, not necessary
		NaturalNumber firstClone = this.clone();
		...
		int i;
		for(i = 0; i < firstClone.coefficients.size(); i++){
			product = product.plus(Multiplier(firstClone.coefficients.get(i), secondClone).timesBaseToThePower(i));
	...
	public NaturalNumber Multiplier(int m, NaturalNumber secondClone){ //this will treat the multiplier as an int and multiplicand as a linked list

		NaturalNumber sProduct = new NaturalNumber(this.base); //list to hold the products of each multiplication

		int carry = 0;
		int tmp;
		int i;

		for (i = 0; i < secondClone.coefficients.size(); i++){ //iterate through the multiplicand for each coefficient of multiplier

			tmp= ( m * secondClone.coefficients.get(i) + carry); 
			int remainder = tmp % base;
			carry = tmp / base;

			sProduct.coefficients.addLast(remainder);

		}

		if (carry != 0){
			sProduct.coefficients.addLast(carry);
		}
	...
	public NaturalNumber  minus(NaturalNumber second) throws Exception{
		...
		//going to borrow code from above to make first and secondClone same length
		...
		for (i = 0; i < first.coefficients.size(); i++){
			...
			if (first.coefficients.get(i) >= secondClone.coefficients.get(i)){
				...
				//add the base from the next number in the next list to subtract and change that number

				tmp = (first.coefficients.get(i) + base) - secondClone.coefficients.get(i);
				int newfirst = (first.coefficients.get(i+1)) -1 ; // subtract one from the next number
				first.coefficients.set(i+1, newfirst); //set the next number to the new number
				difference.coefficients.addLast(tmp);		
			}
	...
	//  --------------- BEGIN SOLUTION (divide) --------------------------
		...
		NaturalNumber compareClone = new NaturalNumber(this.base);
		NaturalNumber divideClone = new NaturalNumber(this.base);
		NaturalNumber divisorClone = divisor.clone();

		int i, j;
		for (i = remainder.coefficients.size() - 1; i >= 0; i--){
			compareClone.coefficients.addFirst(remainder.coefficients.get(i));

			if (compareClone.compareTo(divisorClone) >=0){

				divideClone.coefficients.clear();

				for (j = 1; j <= this.base; j++){

					divideClone = divisorClone.Multiplier(j, divisorClone);

					if (divideClone.compareTo(compareClone) == 0){
						quotient.coefficients.addFirst(j);
						compareClone.coefficients.clear();
						break;

					}
					else if (divideClone.compareTo(compareClone) == 1){
						compareClone = compareClone.minus(divisorClone.Multiplier(j-1, divisorClone));
			...
			if((compareClone.coefficients.getLast() == 0) && (compareClone.coefficients.size() > 0)){
				compareClone.coefficients.removeLast();
			}

		}

		while ((quotient.coefficients.getLast().intValue() == 0) && (quotient.coefficients.size() > 1)){
